...
function NFTDropPage({ collection }: Props) {
    ...
    const mintNft = () => {
        ...
        nftDrop.claimTo(address, quantity).then(async (tx) => {
            ...
            toast('Hurray ... You have claimed NFT', {
                duration: 8000,
                style: {
                    background: 'green',
                    color: 'white',
    ...
    return (
                ...
                <header className='flex items-center justify-between'>
                    <Link href={'/'}>
                        <h1 className='w-52 cursor-pointer text-xl font-extralight sm:w-80'>The
                            {' '}<span className='font-extrabold underline decoration-pink-600'>NAME</span>{' '}
                            NFT's Market Place. </h1>
                    </Link>
        ...
        {/*  content   */}
            ...
            <div className='mt-10 flex flex-1 flex-col items-center space-y-6 text-center
                        lg:space-y-0'>
                <img src={urlFor(collection.mainImage).url()} alt="" className='w-80 object-cover pb-10 lg:h-60'/>
                <h1 className='text-3xl font-bold lg:text-5xl lg:font-extrabold'>{collection.title}</h1>

                {loading ? (
                    <p className='pt-2 text-xl text-green-500 animate-pulse'>Loading .....</p>
                ) : (
        ...
        {/*  button   */}
            ...
            <button onClick={mintNft} disabled={loading || claimedSupply === totalSupply?.toNumber() || !address}
                className='h-16 bg-red-600 text-white rounded-full shadow-xs font-bold disabled:bg-gray-400'>
                {loading ? (
                    <>Loading....</>
